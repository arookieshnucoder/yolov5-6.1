...
"""
    Loss functions
    这个文件是yolov5的损失函数部分。代码量不多，只有300多行，但却是整个项目最难，最精华的部分。
"""
...
def smooth_BCE(eps=0.1):  
    """
        函数功能：一个标签平滑的策略(trick)，是一种在分类/检测问题中，防止过拟合的方法。
        ·@Dragon AI
    ...
    """
        ...
        :params eps: 平滑参数
        :return positive 正样本的标签取值 
                negative label smoothing BCE targets 负样本的标签取值 

                原先的正样本=1 改为 正样本=1.0 - 0.5 * eps  
                原先的负样本=0 改为 负样本=0.5 * eps
        """
...
class FocalLoss(nn.Module):
    """
        函数功能：希望那些hard examples对损失的贡献变大，使网络更倾向于从这些样本上学习。防止由于easy examples过多，主导整个损失函数。
        优点：解决了one-stage object detection中图片中正负样本（前景和背景）不均衡的问题；
              降低简单样本的权重，使损失函数更关注困难样本
                被调用：ComputeLoss类的__init__代替原本的BCEcls（分类损失）和BCEobj（置信度损失）
    """
    ...
    def forward(self, pred, true):
        ...
        # p_t = torch.100轮+YOLOV6.1原型结果(-loss)
        # loss *= self.alpha * (1.000001 - p_t) ** self.gamma  # non-zero power for gradient stability

        # TF implementation https://github.com/tensorflow/addons/blob/v0.7.1/tensorflow_addons/losses/focal_loss.py
        # 当正负样本模型不均衡时，使用
        pred_prob = torch.sigmoid(pred)  # prob from logits ，# 将预测值归一化到0到1之间，表示概率值
        # true=1 p_t=pred_prob    true=0 p_t=1-pred_prob
        p_t = true * pred_prob + (1 - true) * (1 - pred_prob) # p_t # 只留下正样本的预测概率
        # true=1 alpha_factor=self.alpha    true=0 alpha_factor=1-self.alpha
        alpha_factor = true * self.alpha + (1 - true) * (1 - self.alpha) # alpha_t 正负样本的因子
        modulating_factor = (1.0 - p_t) ** self.gamma # 这里代表Focal loss中的指数项，计算正确改指数变小，正样本计算错误，则变大
...
# Todo 整个yolov5的损失计算 修改IOU的地方
class ComputeLoss: # 整个yolov5的损失计算
    """
    ...
    def __init__(self, model, autobalance=False):
        ...
        # 类别标签平滑  eps=0代表不做标签平滑-> cp=1 cn=0  eps!=0代表做标签平滑 cp代表positive的标签值 cn代表negative的标签值
    ...
    def build_targets(self, p, targets): # 构建目标数据方法
        """
            函数功能：为每个feature map上的三个anchor筛选相应的正样本的（ground true）。
                        筛选条件是比较GT和anchor的宽比和高比，大于一定的阈值就是负样本，反之正样本。
                        筛选到的各个feature的各个anchor的正样本信息（image_index, anchor_index, gridy, gridx），传入__call__函数，
        ...
        for i in range(self.nl):    # self.nl: number of detection layers   Detect的个数 = 3
            ...
            t = targets * gain # 将目标边框的0到1的范围下，则将改目标框对应的原图的一个anchoes的尺度下
            ...
            if nt:  # 开始匹配  Matches
                ...
                gxy = t[:, 2:4]  # grid xy 取target中心的坐标xy(相对feature map左上角的坐标)，缩小到当前tensor尺度下的xy
            ...
            gxy = t[:, 2:4]  # target的xy
            gwh = t[:, 4:6]  # target的wh
            gij = (gxy - offsets).long()    # 预测真实框的网格所在的左上角坐标(有左上右下的网格)，正采样的坐标
