...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
        weights=ROOT / 'yolov5s.pt',  # 要转换的权重文件pt地址 默认='../weights/best.pt'
        imgsz=(640, 640),  # 输入模型的图片size=(height, width) 默认=[640, 640]
        batch_size=1,  # batch大小 默认=1
        device='cpu',  # 模型运行设备 cuda device, i.e. 0 or 0,1,2,3 or cpu 默认=cpu
        include=('torchscript', 'onnx'),  # 要将pt文件转为什么格式 可以为单个原始也可以为list 默认=['torchscript', 'onnx', 'coreml']
        half=False,  #  是否使用半精度FP16export转换 默认=False
        inplace=False,  # 是否set YOLOv5 Detect() inplace=True  默认=False
        train=False,  # 是否开启model.train() mode 默认=True  coreml转换必须为True
        optimize=False,  # TorchScript转化参数 是否进行移动端优化  默认=False
        int8=False,  # CoreML/TF INT8 quantization
        dynamic=False,  # ONNX转换参数  dynamic_axes  ONNX转换是否要进行批处理变量  默认=False，ONNX/TF: dynamic axes
        simplify=False,  # ONNX转换参数 是否简化onnx模型  默认=False
    ...
    im = torch.zeros(batch_size, 3, *imgsz).to(device)  # # 自定义一张图片 输入model，image size(1,3,320,192) BCHW iDetection
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s.pt', help='model.pt path(s)') # 要转换的权重文件pt地址 默认='../weights/best.pt'
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[640, 640], help='image (h, w)') # 输入模型的图片size=(height, width) 默认=[640, 640]
    parser.add_argument('--batch-size', type=int, default=1, help='batch size') # batch大小 默认=1
    parser.add_argument('--device', default='cpu', help='cuda device, i.e. 0 or 0,1,2,3 or cpu') # 模型运行设备 cuda device, i.e. 0 or 0,1,2,3 or cpu 默认=cpu
    parser.add_argument('--half', action='store_true', help='FP16 half-precision export') # 是否使用半精度FP16export转换 默认=False
    parser.add_argument('--inplace', action='store_true', help='set YOLOv5 Detect() inplace=True')  # 是否set YOLOv5 Detect() inplace=True  默认=False
    parser.add_argument('--train', action='store_true', help='model.train() mode')  # 是否开启model.train() mode 默认=True  coreml转换必须为True
    parser.add_argument('--optimize', action='store_true', help='TorchScript: optimize for mobile') # TorchScript转化参数 是否进行移动端优化  默认=False
    parser.add_argument('--int8', action='store_true', help='CoreML/TF INT8 quantization')
    parser.add_argument('--dynamic', action='store_true', help='ONNX/TF: dynamic axes') # ONNX转换参数  dynamic_axes  ONNX转换是否要进行批处理变量  默认=False
    parser.add_argument('--simplify', action='store_true', help='ONNX: simplify model') # ONNX转换参数 是否简化onnx模型  默认=False
